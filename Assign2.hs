...
fun1 = [Fun ("main",["x","y"],(ADD (VAR "x") (VAR "y")))]
...
testTable = ([("y",1),("x",0)],[("main",9)],2,1)

rename::(Printer a, Printer b) => (([(String,Integer)],[(String, Integer)],Integer, Integer),[Fun a b]) -> (([(String,Integer)],[(String, Integer)],Integer, Integer), [Fun a b])
--rename (table, funclist) = map (\(Fun (name,args,exp)) -> (Fun ((replaceName name), (replaceArgs args), (replaceExps exp))) ) funcList
rename (table, funclist) = (table, map (\func -> replaceName table func) funclist) 


replaceName::(Printer a, Printer b) =>([(String,Integer)],[(String, Integer)],Integer, Integer) -> (Fun a b) -> (Fun a b)
replaceName (_,fList,_,fNum) (Fun (name, args,exp)) = case ((printer name) `elem` (map fst fList)) of 
